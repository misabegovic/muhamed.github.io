...
This lands right next to a thought I've had — [is Ruby dead, and is it time to build cathedrals in Go?](/brain/is-ruby-dead-cathedrals-in-go/) — more a passing question than a deep position, and one that mostly came out of a conversation with a friend who's building cathedrals in Go these days. It's not just Battley: that friend doesn't like DHH either, and doesn't think Rails and Ruby are tools for the future. My framing was technical — cost, speed, guard rails. Battley's is about who holds the project. They all point the same way: the biggest risk in a framework isn't performance, it's who controls it.

And that closing line about Claude opens a thread that goes well beyond Ruby. Two more pieces from the same July land on it from opposite ends.
