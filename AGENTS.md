...
4. **Stop and shape.** If the user asks for a structural change, do not execute. Respond with a draft ADR and stop for approval. Structural changes include repo layout, `brain.config.yml`, `.github/workflows/`, `AGENTS.md`, repo conversion, and onboarding/removing repos. The active constraint `adr-before-structural-changes` makes this a `must`.
...
3. **Link intent.** Every PR or commit must cite the ADR/PRD/bet/record it implements.
...
5. **Same-repo exception.** If the pi-brain clone also maintains the code (converted repo), the decision record may be included in the same PR or commit as the code change.
6. **One bet per PR.** Avoid PRs or commits that implement multiple unrelated bets.
